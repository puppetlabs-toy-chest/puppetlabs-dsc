...
Puppet::Type.newtype(:dsc_xscsrserverupdate) do
  ...
  newparam(:dscmeta_resource_friendly_name) do
    desc "A read-only value that is the DSC Resource Friendly Name ('xSCSRServerUpdate')."

    validate do |value|
      fail 'dscmeta_resource_friendly_name is read-only' if value != 'xSCSRServerUpdate'
    end
  ...
  newparam(:dscmeta_resource_name) do
    desc "A read-only value that is the DSC Resource Name ('MSFT_xSCSRServerUpdate')."

    validate do |value|
      fail 'dscmeta_resource_name is read-only' if value != 'MSFT_xSCSRServerUpdate'
    end
  ...
  newparam(:dscmeta_module_name) do
    desc "A read-only value that is the DSC Module Name ('xSCSR')."

    validate do |value|
      fail 'dscmeta_module_name is read-only' if value != 'xSCSR'
    end
