...
Puppet::Type.newtype(:dsc_auditpolicycsv) do
  ...
  def dscmeta_resource_friendly_name; 'AuditPolicyCsv' end
  def dscmeta_resource_name; 'MSFT_AuditPolicyCsv' end
  ...
  # IsMandatory:  False
  # Values:       None
  newparam(:dsc_csvpath) do
    def mof_type; 'string' end
    def mof_is_embedded?; false end
    desc "CsvPath - Path to a CSV backup of Auditing settings"
    validate do |value|
      unless value.kind_of?(String)
        fail("Invalid value '#{value}'. Should be a string")
      end
