...
Puppet::Type.newtype(:dsc_xspserviceappsecurity) do
  require Pathname.new(__FILE__).dirname + '../../' + 'puppet/type/base_dsc'
  require Pathname.new(__FILE__).dirname + '../../puppet_x/puppetlabs/dsc_type_helpers'

    class PuppetX::Dsc::TypeHelpers
      def self.validate_MSFT_xSPServiceAppSecurityEntry(mof_type_map, name, value)
        ...
        lowkey_hash.keys.each do |key|
          if lowkey_hash[key]
            validate_mof_type(mof_type_map[key], 'MSFT_xSPServiceAppSecurityEntry', key, lowkey_hash[key])
          end
        end
      end
    end
  ...
  @doc = %q{
    The DSC xSPServiceAppSecurity resource type.
    Automatically generated from
    'xSharePoint/Modules/xSharePoint/DSCResources/MSFT_xSPServiceAppSecurity/MSFT_xSPServiceAppSecurity.schema.mof'
  ...
  def dscmeta_resource_friendly_name; 'xSPServiceAppSecurity' end
  def dscmeta_resource_name; 'MSFT_xSPServiceAppSecurity' end
  ...
  # Name:         Members
  # Type:         MSFT_xSPServiceAppSecurityEntry[]
  # IsMandatory:  False
  # Values:       None
  newparam(:dsc_members, :array_matching => :all) do
    def mof_type; 'MSFT_xSPServiceAppSecurityEntry[]' end
    ...
    validate do |value|
      ...
      (value.kind_of?(Hash) ? [value] : value).each_with_index do |v, i|
        ...
        PuppetX::Dsc::TypeHelpers.validate_MSFT_xSPServiceAppSecurityEntry(mof_type_map, "Members", v)
      end
    end
    munge do |value|
      value.kind_of?(Hash) ?
  ...
  newparam(:dsc_memberstoinclude, :array_matching => :all) do
    ...
    validate do |value|
      ...
      (value.kind_of?(Hash) ? [value] : value).each_with_index do |v, i|
        ...
        PuppetX::Dsc::TypeHelpers.validate_MSFT_xSPServiceAppSecurityEntry(mof_type_map, "MembersToInclude", v)
      end
    end
    munge do |value|
      value.kind_of?(Hash) ?
...
Puppet::Type.type(:dsc_xspserviceappsecurity).provide :powershell, :parent => Puppet::Type.type(:base_dsc).provider(:powershell) do
