...
Puppet::Type.newtype(:dsc_xexchmailboxserver) do
  ...
  newparam(:dscmeta_resource_friendly_name) do
    desc "A read-only value that is the DSC Resource Friendly Name ('xExchMailboxServer')."

    validate do |value|
      fail 'dscmeta_resource_friendly_name is read-only' if value != 'xExchMailboxServer'
    end

    defaultto 'xExchMailboxServer'
  end

  newparam(:dscmeta_resource_name) do
    desc "A read-only value that is the DSC Resource Name ('MSFT_xExchMailboxServer')."

    validate do |value|
      fail 'dscmeta_resource_name is read-only' if value != 'MSFT_xExchMailboxServer'
    end

    defaultto 'MSFT_xExchMailboxServer'
  end
