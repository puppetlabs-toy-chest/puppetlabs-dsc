...
Puppet::Type.newtype(:dsc_xmppreference) do
  ...
  newparam(:dscmeta_resource_friendly_name) do
    desc "A read-only value that is the DSC Resource Friendly Name ('xMpPreference')."

    validate do |value|
      fail 'dscmeta_resource_friendly_name is read-only' if value != 'xMpPreference'
    end
  ...
  newparam(:dscmeta_resource_name) do
    desc "A read-only value that is the DSC Resource Name ('MSFT_xMpPreference')."

    validate do |value|
      fail 'dscmeta_resource_name is read-only' if value != 'MSFT_xMpPreference'
    end
  ...
  newparam(:dscmeta_module_name) do
    desc "A read-only value that is the DSC Module Name ('xDefender')."

    validate do |value|
      fail 'dscmeta_module_name is read-only' if value != 'xDefender'
    end
