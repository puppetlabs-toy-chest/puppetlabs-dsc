...
Puppet::Type.newtype(:dsc_xwebapplication) do
  ...
  # Type:         string[]
  # IsMandatory:  False
  # Values:       ["Ssl", "SslNegotiateCert", "SslRequireCert"]
  newparam(:dsc_sslflags, :array_matching => :all) do
    def mof_type; 'string[]' end
    def mof_is_embedded?; false end
    desc "SslFlags - SSLFlags for the application Valid values are Ssl, SslNegotiateCert, SslRequireCert."
    validate do |value|
      unless value.kind_of?(Array) || value.kind_of?(String)
        fail("Invalid value '#{value}'. Should be a string or an array of strings")
      end
      if value.kind_of?(Array)
        unless (['Ssl', 'ssl', 'SslNegotiateCert', 'sslnegotiatecert', 'SslRequireCert', 'sslrequirecert'] & value).count == value.count
          fail("Invalid value #{value}. Valid values are Ssl, SslNegotiateCert, SslRequireCert")
        end
      end
      if value.kind_of?(String)
        unless ['Ssl', 'ssl', 'SslNegotiateCert', 'sslnegotiatecert', 'SslRequireCert', 'sslrequirecert'].include?(value)
          fail("Invalid value #{value}. Valid values are Ssl, SslNegotiateCert, SslRequireCert")
        end
      end
    end
