...
Puppet::Type.newtype(:dsc_xexchtransportservice) do
  ...
  newparam(:dscmeta_resource_friendly_name) do
    desc "A read-only value that is the DSC Resource Friendly Name ('xExchTransportService')."

    validate do |value|
      fail 'dscmeta_resource_friendly_name is read-only' if value != 'xExchTransportService'
    end

    defaultto 'xExchTransportService'
  end

  newparam(:dscmeta_resource_name) do
    desc "A read-only value that is the DSC Resource Name ('MSFT_xExchTransportService')."

    validate do |value|
      fail 'dscmeta_resource_name is read-only' if value != 'MSFT_xExchTransportService'
    end

    defaultto 'MSFT_xExchTransportService'
  end
