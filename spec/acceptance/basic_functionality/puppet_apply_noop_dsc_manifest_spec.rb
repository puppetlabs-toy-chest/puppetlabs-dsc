...
describe 'Apply DSC Resource Manifest in "noop" Mode Using "puppet apply"' do
  context 'Apply noop manifest and verify no files created' do
    it 'runs noop on manifest does not create files after noop manifest application' do
      ...
      result = assert_dsc_resource('File',
                                   'PSDesiredStateConfiguration',
                                   { DestinationPath: 'C:/test/test.txt', Contents: 'catcat' },
                                   # Passing true to expect_ps_failures. We expect this to fail.
                                   true,
                                   # Passing true to expect_dsdc_failures. We expect dsc to fail to find the resource.
                                   true)
      result_string = strip_crln(result.stdout)
      expect(result_string).to match(%r{cannot find the path specified})
    end
  end
end
