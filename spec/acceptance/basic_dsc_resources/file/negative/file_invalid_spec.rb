...
describe 'Negative file tests' do
  ...
  context 'DSC File resource with invalid data type for Force' do
    ...
    it_behaves_like 'an error in a puppet run', dsc_manifest, %r{invalid value: cows}
  end
  ...
  context 'DSC File resource with invalid data for DestinationPath' do
    ...
    it_behaves_like 'an error in a puppet run', dsc_manifest, %r{The system cannot find the path specified.}
  end
  ...
  context 'DSC File resource with invalid data for SourcePath' do
    ...
    it_behaves_like 'an error in a puppet run', dsc_manifest, %r{Error:.*SourcePath must be accessible for current configuration.}
  end
end
