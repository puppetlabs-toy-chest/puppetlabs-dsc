...
Puppet::Type.newtype(:dsc_puppetfakeresource) do
  ...
  validate do
    raise('dsc_importantstuff is a required attribute') if self[:dsc_importantstuff].nil?
  end

  def dscmeta_resource_friendly_name
    'PuppetFakeResource'
  end
  ...
  def dscmeta_module_name
    'PuppetFakeResource'
  end

  def dscmeta_module_version
    '1.0'
  end
  ...
  newparam(:dsc_ensure) do
    ...
    validate do |value|
      ...
      unless ['Present', 'present', 'Absent', 'absent'].include?(value)
        raise("Invalid value '#{value}'. Valid values are Present, Absent")
      end
    end
  end
