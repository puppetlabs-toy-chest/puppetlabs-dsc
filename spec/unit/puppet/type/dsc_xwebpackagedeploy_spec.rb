...
describe Puppet::Type.type(:dsc_xwebpackagedeploy) do
  ...
  it 'should accept dsc_ensure predefined value present and update ensure with this value (ensure end value should be a symbol)' do
    dsc_xwebpackagedeploy[:dsc_ensure] = 'present'
    expect(dsc_xwebpackagedeploy[:ensure]).to eq(dsc_xwebpackagedeploy[:dsc_ensure].downcase.to_sym)
  end
  ...
  describe "powershell provider tests" do
    ...
    describe "when dsc_ensure is 'absent'" do
      ...
      before(:each) do
        dsc_xwebpackagedeploy.original_parameters[:dsc_ensure] = 'absent'
        dsc_xwebpackagedeploy[:dsc_ensure] = 'absent'
        @provider = described_class.provider(:powershell).new(dsc_xwebpackagedeploy)
      end

      it "should update :ensure to :absent" do
        expect(dsc_xwebpackagedeploy[:ensure]).to eq(:absent)
      end
