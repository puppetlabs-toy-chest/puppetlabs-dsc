...
describe Puppet::Type.type(:dsc_xspbcsserviceapp) do
  ...
  it 'should default to ensure => present' do
    expect(dsc_xspbcsserviceapp[:ensure]).to eq :present
  end
  ...
  it 'should accept dsc_ensure predefined value Present' do
    dsc_xspbcsserviceapp[:dsc_ensure] = 'Present'
    expect(dsc_xspbcsserviceapp[:dsc_ensure]).to eq('Present')
  end
  ...
  it 'should accept dsc_ensure predefined value present and update ensure with this value (ensure end value should be a symbol)' do
    dsc_xspbcsserviceapp[:dsc_ensure] = 'present'
    expect(dsc_xspbcsserviceapp[:ensure]).to eq(dsc_xspbcsserviceapp[:dsc_ensure].downcase.to_sym)
  end

  it 'should accept dsc_ensure predefined value Absent' do
    dsc_xspbcsserviceapp[:dsc_ensure] = 'Absent'
    expect(dsc_xspbcsserviceapp[:dsc_ensure]).to eq('Absent')
  end
  ...
  it 'should not accept values not equal to predefined values' do
    expect{dsc_xspbcsserviceapp[:dsc_ensure] = 'invalid value'}.to raise_error(Puppet::ResourceError)
  end

  it 'should not accept array for dsc_ensure' do
    expect{dsc_xspbcsserviceapp[:dsc_ensure] = ["foo", "bar", "spec"]}.to raise_error(Puppet::ResourceError)
  end

  it 'should not accept boolean for dsc_ensure' do
    expect{dsc_xspbcsserviceapp[:dsc_ensure] = true}.to raise_error(Puppet::ResourceError)
  end

  it 'should not accept int for dsc_ensure' do
    expect{dsc_xspbcsserviceapp[:dsc_ensure] = -16}.to raise_error(Puppet::ResourceError)
  end
  ...
  describe "powershell provider tests" do
    ...
    describe "when dsc_ensure is 'present'" do
      ...
      before(:each) do
        dsc_xspbcsserviceapp.original_parameters[:dsc_ensure] = 'present'
        dsc_xspbcsserviceapp[:dsc_ensure] = 'present'
        @provider = described_class.provider(:powershell).new(dsc_xspbcsserviceapp)
      end
    ...
    describe "when dsc_ensure is 'absent'" do
      ...
      before(:each) do
        dsc_xspbcsserviceapp.original_parameters[:dsc_ensure] = 'absent'
        dsc_xspbcsserviceapp[:dsc_ensure] = 'absent'
        @provider = described_class.provider(:powershell).new(dsc_xspbcsserviceapp)
      end

      it "should update :ensure to :absent" do
        expect(dsc_xspbcsserviceapp[:ensure]).to eq(:absent)
      end
