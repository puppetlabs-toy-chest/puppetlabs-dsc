...
describe Puppet::Type.type(:dsc_xrdremoteapp) do
  ...
  it 'should accept string-like int for dsc_iconindex' do
  ...
  describe "dsc_configuration provider tests" do
    ...
    it "should successfully instanciate the provider" do
      described_class.provider(:dsc_configuration).new(dsc_xrdremoteapp)
    end
    ...
    describe "when dscmeta_import_resource is false" do
      ...
      it "should compute powershell dsc test script without Import-DscResource" do
        dsc_xrdremoteapp[:dscmeta_import_resource] = false
  ...
  describe "mof provider tests" do
    ...
    it "should successfully instanciate the provider" do
      described_class.provider(:dsc_mof).new(dsc_xrdremoteapp)
    end

    before(:each) do
      @provider = described_class.provider(:dsc_mof).new(dsc_xrdremoteapp)
    end

    it "should successfully build mof file" do
#     expect(@provider.mof_test_content).to match(/instance of MSFT_xRDRemoteApp as $MSFT_xRDRemoteApp1ref$/)
      expect(@provider.mof_test_content).to match(/instance of MSFT_xRDRemoteApp/)
    end
