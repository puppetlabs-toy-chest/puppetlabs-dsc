def locate_dsc_module(expect_ps_failure)
  # Init
  module_paths = run_shell('puppet config print modulepath').stdout.gsub("\n", '').gsub("\r", '').split(';')
...
def locate_dsc_vendor_resources(expect_ps_failure)
  # Init
  libdir_path = run_shell("puppet config print libdir").stdout.rstrip.gsub('\\', '/')
...
def assert_dsc_resource(dsc_resource_type, dsc_module, expect_ps_failure = false, expect_dsc_failure = false, dsc_properties)
